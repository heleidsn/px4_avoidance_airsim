...
class CNN_GAP(BaseFeaturesExtractor):
    ...
    def __init__(self, observation_space: gym.spaces.Box, features_dim: int = 256, state_feature_dim=0):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(1, 8, kernel_size=3, stride=1, padding='same'),  # 60 100
            # nn.BatchNorm2d(8),
            nn.Tanh(),
            nn.MaxPool2d(2, 2)  # [1, 8, 40, 50]  # 20, 25   30， 50
        )
        ...
        self.conv2 = nn.Sequential(
            ...
            # nn.BatchNorm2d(16),
            nn.Tanh(),
            nn.MaxPool2d(2, 2),  # [1, 8, 20, 25]  # 15， 25
            # nn.MaxPool2d(kernel_size=(10, 5), stride=(10, 5)),  # [1, 8, 10, 12]
            # nn.Flatten(),
            # nn.Tanh()
        ...
        self.conv3 = nn.Sequential(
            nn.Conv2d(8, 8, kernel_size=3,
                      stride=1, padding='same'),  # 20, 25
            nn.Tanh(),
            nn.MaxPool2d(kernel_size=(5, 5), stride=(5, 5)),  # [1, 8, 10, 12]  # 2， 5
            # nn.Tanh(),
            nn.Flatten()
        )
        # self.gap_layer = nn.AvgPool2d(kernel_size=(5, 3), stride=1)  # 2, 4

        # self.batch_layer = nn.BatchNorm1d(self.feature_num_cnn)
        self.fc = nn.Linear(8*3*5, self.feature_num_cnn)
    ...
    def forward(self, observations: th.Tensor) -> th.Tensor:
        ...
        self.fc_out = self.fc(self.layer_3_out)

        cnn_feature = th.tanh(self.fc_out)  # [1, 8, 1, 1]
